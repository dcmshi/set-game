...
// The suit colour is the inherited `color` of the .color-<suit> class on the
// card, so painting with currentColor lets a palette swap happen entirely in CSS.
function ShapeSvg({ shape, shading, color }: { shape: Shape; shading: Shading; color: Color }) {
  const fill =
    shading === 'solid' ? 'currentColor' : shading === 'striped' ? `url(#stripes-${color})` : 'none';
  return (
    <svg className="shape" viewBox="0 0 50 100" aria-hidden="true">
      {shape === 'oval' ? (
        <rect x="6" y="6" width="38" height="88" rx="19" fill={fill} stroke="currentColor" strokeWidth="4" />
      ) : (
        <path
          d={SHAPE_PATH[shape]}
          fill={fill}
